<?php
/**
 * This file is a customized copy of https://github.com/PHPOffice/PHPWord/blob/fd156bf12a6dbfb4ba1bc2b21b0ed7da551c2537/src/PhpWord/Shared/Html.php
 *  
...
class E2w_Html
{
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('PhpOffice\PhpWord\Shared\E2w_Html', $method), $arguments);
    ...
    private static function parseImage($node, $element, &$styles)
    {
		...
		if ( is_numeric( strpos( $classes, 'alignnone' ) ) ){
			$style['wrappingStyle'] = 'inline';
		} else {
			$style['wrappingStyle'] = 'square';
			$style['positioning'] = 'absolute';
			$style['posHorizontalRel'] = 'margin';
			$style['posVerticalRel'] = 'line';
			if ( is_numeric( strpos( $classes, 'alignleft' ) ) )
				$style['posHorizontal'] = \PhpOffice\PhpWord\Style\Image::POSITION_HORIZONTAL_LEFT;
			if ( is_numeric( strpos( $classes, 'alignright' ) ) )
				$style['posHorizontal'] = \PhpOffice\PhpWord\Style\Image::POSITION_HORIZONTAL_RIGHT;
			if ( is_numeric( strpos( $classes, 'aligncenter' ) ) )
				$style['posHorizontal'] = \PhpOffice\PhpWord\Style\Image::POSITION_HORIZONTAL_CENTER;
		}
		
        $newElement = $element->addImage( $node->getAttribute('src'), $style );
